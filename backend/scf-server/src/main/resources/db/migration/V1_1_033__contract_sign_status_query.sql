...
WITH perms(id, code, name, method, path) AS (
  VALUES
    ('PERM_CONTRACT_SIGN_STATUS_QUERY', 'CONTRACT_SIGN_STATUS_QUERY', '合同签章主动查单', 'POST', '/integrations/contracts/sign/by-ref/*/query-status')
)
...
FROM (
    SELECT 'ROLE_PLATFORM_ADMIN' AS role_id, 'CONTRACT_SIGN_STATUS_QUERY' AS permission_code
    UNION ALL SELECT 'ROLE_FUNDING', 'CONTRACT_SIGN_STATUS_QUERY'
) rp
